...
class Tello:
    ...
    def _video_thread(self):
        # wait to be fixed
        stream_address = 'udp://'+self.tello_ip+':11111'
        
        vid_cap = cv2.VideoCapture(stream_address)
        
        vid_cap.set(cv2.CAP_PROP_BUFFERSIZE,3)
        vid_cap.set(cv2.CAP_PROP_FRAME_WIDTH,1280)
        vid_cap.set(cv2.CAP_PROP_FRAME_HEIGHT,720)
        vid_cap.set(cv2.CAP_PROP_FPS,24)
        vid_cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('H','2','6','4'))
        ...
        while self.stream_state:
            ret, self.last_frame = vid_cap.read()
            if not ret:
                print("Error: Failed to read frame.")
                break
            processed_frame ,name, position = recognize(self.last_frame,self.debug)
            if self.debug:
